// ============================================================
// BestMobilePhone Search Service
// Scrapes BestMobilePhone search results
// ============================================================
...
export class BestmobilephoneSearchService {
  ...
  async searchProduct(page: Page, productQuery: string): Promise<AmazonSearchResult[]> {
    ...
    logger.info(`Visiting BestMobilePhone: ${searchUrl}`);
    ...
    const results = await page.evaluate((maxResults) => {
      const items: any[] = [];
      const cards = document.querySelectorAll('.product-grid-item, .product-item, .item');
    ...
    logger.info(`Found ${results.length} results on BestMobilePhone.`);
    return results;
  }
}
