// ============================================================
// Scorptec Search Service
// Scrapes Scorptec search results for laptops
// ============================================================
...
export class ScorptecSearchService {
  ...
  async searchProduct(page: Page, productQuery: string): Promise<AmazonSearchResult[]> {
    const searchUrl = `https://${this.domain}/product/search?s=${encodeURIComponent(productQuery)}`;
    logger.info(`Visiting Scorptec: ${searchUrl}`);
    ...
    const results = await page.evaluate((maxResults) => {
      const items: any[] = [];
      const cards = document.querySelectorAll('.product-list-item, .product-item, .card, [data-test="product-card"]');
      
      for (const card of Array.from(cards).slice(0, maxResults)) {
        const titleEl = card.querySelector('.product-name, h2, h3, a.title');
    ...
    logger.info(`Found ${results.length} results on Scorptec.`);
    return results;
  }
}
